...
CREATE TABLE user (
  ...
  user_mark mediumint(9) COMMENT '用户积分',
  user_rank_id tinyint(3) COMMENT '用户等级',
  ...
  user_lock tinyint(3) COMMENT '是否锁定，0为不锁定，1为锁定',
  user_freeze tinyint(3) COMMENT '是否冻结，0为不冻结，1为冻结',
...
CREATE TABLE user_rank (
  id mediumint(5) NOT NULL AUTO_INCREMENT COMMENT '自增ID',
...
###################
CREATE TABLE friend (
  id smallint(5) NOT NULL AUTO_INCREMENT COMMENT '自增ID',
...
CREATE TABLE system_message (
  id mediumint(8) NOT NULL AUTO_INCREMENT COMMENT '系统通知ID',
...
CREATE TABLE ad (
  id smallint(5) NOT NULL AUTO_INCREMENT COMMENT '自增ID',
  position_id smallint(5) COMMENT '0,站外广告;从1开始代表的是该广告所处的广告位,同表ad_postition中的字段position_id的值',
  media_type tinyint(3) DEFAULT 0 COMMENT '广告类型,0图片;1flash;2代码3文字',
  ...
  start_time int(13) DEFAULT 0 COMMENT '广告开始时间',
  end_time int(13) DEFAULT 0 COMMENT '广告结束时间',
  link_man varchar(60) COMMENT '广告联系人',
  link_email varchar(60) COMMENT '广告联系人的邮箱',
  link_phone varchar(60) COMMENT '广告联系人得电话',
  click_count mediumint(8) DEFAULT 0 COMMENT '广告点击次数',
  enabled tinyint(3) DEFAULT 1 COMMENT '该广告是否关闭;1开启; 0关闭; 关闭后广告将不再有效',
  PRIMARY KEY (id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 ;
...
CREATE TABLE stay_message (
  id smallint(5) NOT NULL AUTO_INCREMENT COMMENT '留言表自增ID',
  user_id mediumint(8) NOT NULL COMMENT '用户ID',
  ...
  message_stay_time int(13) COMMENT '留言时间',
...
CREATE TABLE visitor (
  ...
  visitor_time int(13) COMMENT '来访时间',
  user_id mediumint(8) NOT NULL COMMENT '被访用户ID',
  visitor_ip varchar(15) COMMENT '访客IP地址',
  type_id int(3) COMMENT '访问板块ID',
  where_id mediumint(8) COMMENT '查看某板块的某个子项目，如查看相册板块的第3个相册，该ID对应该相册的ID号',
  PRIMARY KEY (id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 ;
...
CREATE TABLE shuoshuo (
  ...
  user_id mediumint(8) NOT NULL COMMENT '用户ID',
  shuo_time int(13) DEFAULT 0 COMMENT '发布时间',
  shuo_ip varchar(15) COMMENT '说说发布时的IP地址',
  shuoshuo varchar(255) NOT NULL COMMENT '说说内容',
  type_id tinyint(3) DEFAULT 3 COMMENT '栏目ID,默认为3',
  PRIMARY KEY (id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 ;
...
CREATE TABLE photo_sort (
  ...
  user_id mediumint(8) COMMENT '所属用户ID',
  img_sort_question varchar(255) COMMENT '访问问题',
  img_sort_answer varchar(128) COMMENT '访问问题的答案',
  type_id int(3) DEFAULT 1 COMMENT '默认1表示相册板块',
  top_pic_src mediumint(8) COMMENT '封面图片的路径',
  PRIMARY KEY (id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 ;
...
CREATE TABLE photos (
  ...
  user_id mediumint(8) COMMENT '所属用户ID',
  sort_id mediumint(8) COMMENT '所属相册ID',
  upload_time int(13) COMMENT '图片上传时间',
...
CREATE TABLE article (
  id smallint(5) NOT NULL AUTO_INCREMENT COMMENT '日志自增ID号',
  article_name varchar(128) COMMENT '文章名称',
  article_time int(13) COMMENT '发布时间',
  article_ip varchar(15) COMMENT '发布IP',
  article_click int(10) COMMENT '查看人数',
  sort_article_id mediumint(8) COMMENT '所属分类',
  user_id mediumint(8) COMMENT '所属用户ID',
...
CREATE TABLE phone_message (
  ...
  send_time int(13) COMMENT '发送时间',
  user_id mediumint(8) COMMENT '用户ID',
  PRIMARY KEY (id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 ;